...
object ChainMapStateStream {
  private val logger: Logger = LoggerFactory.getLogger(ChainMapStateStream.getClass)
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mapvalues = kafaDirectStream.map(line =>dealKafkaJson(line.value()))
      ...
      }).filter(_._1 != "").mapWithState(StateSpec.function((key: String, one: Option[JSONObject], state: State[String])=>{
      var stateJsonObj=JSON.parseObject(state.getOption().getOrElse(initJson))
      val currentJsonObj=one.get
      stateJsonObj=mergeTrace(currentJsonObj,stateJsonObj)
      state.update(stateJsonObj.toJSONString)
      (key,one)
    })).map(x => x._2)
    ...
    mapvalues.foreachRDD(rdd => {
      ...
      val afterRdd=rdd.filter(pair=>{pair.get.getLong("timestamp") / 1000 > currentTimestamp.value})
      //      结果写入es
      afterRdd.coalesce(500).foreachPartition(partitionOfRecords => partitionOfRecords.foreach(pair =>EsClientUtil.bulkListAdd(pair.get.toJSONString)))
